...
const img = require('../../../assets/images/card_alphabets.png');

export default function AlphabetsCard({ onPress }: { onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [s.card, { opacity: pressed ? 0.85 : 1, transform: [{ scale: pressed ? 0.95 : 1 }] }]}>
      ...
      <View style={[s.footer, { backgroundColor: '#E3F2FD' }]}>
        <Text style={s.name}>Alphabets</Text>
        <Text style={s.desc}>Learn A to Z</Text>
      </View>
    </Pressable>
  );
}
...
const s = StyleSheet.create({
  ...
  imgBox: { height: 120, backgroundColor: '#E3F2FD' },
