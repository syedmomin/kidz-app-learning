...
const img = require('../../../assets/images/card_numbers.png');

export default function NumbersCard({ onPress }: { onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [s.card, { opacity: pressed ? 0.85 : 1, transform: [{ scale: pressed ? 0.95 : 1 }] }]}>
      ...
      <View style={[s.footer, { backgroundColor: '#FFFDE7' }]}>
        <Text style={s.name}>Count It!</Text>
        <Text style={s.desc}>Tap & count</Text>
      </View>
    </Pressable>
  );
}
...
const s = StyleSheet.create({
  ...
  imgBox: { height: 120, backgroundColor: '#FFFDE7' },
