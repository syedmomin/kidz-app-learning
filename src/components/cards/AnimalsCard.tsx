...
const img = require('../../../assets/images/card_animals.png');

export default function AnimalsCard({ onPress }: { onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [s.card, { opacity: pressed ? 0.85 : 1, transform: [{ scale: pressed ? 0.95 : 1 }] }]}>
      ...
      <View style={[s.footer, { backgroundColor: '#E8F5E9' }]}>
        <Text style={s.name}>Animals</Text>
        <Text style={s.desc}>Sounds & names</Text>
      </View>
    </Pressable>
  );
}
...
const s = StyleSheet.create({
  ...
  imgBox:   { height: 120, backgroundColor: '#E8F5E9' },
  ...
  newBadge: { position: 'absolute', top: 8, right: 8, zIndex: 10, backgroundColor: C.coral, borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3 },
