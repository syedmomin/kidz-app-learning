...
const MIX_TABLE: Record<string, Mix> = {
  ...
  // New Complex Mixes (Tier 4 & 5)
  'blue+yellow':   { name: 'Green',    emoji: '🌿', color: '#4CAF50' },
  'orange+white':  { name: 'Peach',    emoji: '🍑', color: '#FFCBA4' },
  'green+white':   { name: 'Mint',     emoji: '🌱', color: '#A8E6B0' },
  'purple+white':  { name: 'Lavender', emoji: '💜', color: '#C8A2D8' },
  'orange+black':  { name: 'Sienna',   emoji: '🍂', color: '#A0522D' },
  'green+black':   { name: 'Forest',   emoji: '🌲', color: '#1F4F1F' },
  'purple+black':  { name: 'Plum',     emoji: '🍆', color: '#5A2A5A' },
  'red+orange':    { name: 'Vermillion', emoji: '🏮', color: '#E34234' },
  'blue+green':    { name: 'Cyan',     emoji: '💎', color: '#00FFFF' },
  'yellow+green':  { name: 'Chartreuse', emoji: '🍐', color: '#7FFF00' },
  'red+purple':    { name: 'Amethyst', emoji: '💎', color: '#9966CC' },
  'blue+purple':   { name: 'Violet',   emoji: '🪻', color: '#8F00FF' },
};
...
function DropletButton({ hue, locked, onPress }: { hue: Hue; locked: boolean; onPress: (cx: number) => void }) {
  ...
  const handlePress = () => {
    if (locked) return;
    ref.current?.measure((_x, _y, _w, _h, pageX, _pageY) => {
      Animated.sequence([
...
function buildRounds(): string[] {
  const list: string[] = [];
  // Build a progressive sequence
  list.push(...shuffle(TIER_1), ...shuffle(TIER_1));
  list.push(...shuffle(TIER_2), ...shuffle(TIER_2));
  list.push(...shuffle(TIER_3), ...shuffle(TIER_3));
  list.push(...shuffle(TIER_4), ...shuffle(TIER_4));
  list.push(...shuffle(TIER_5), ...shuffle(TIER_5));
  
  // Fill up to 500
  const allPossible = [...TIER_1, ...TIER_2, ...TIER_3, ...TIER_4, ...TIER_5];
  while (list.length < 500) {
    list.push(...shuffle(allPossible));
  }
  ...
  for (let i = 1; i < list.length; i++) {
    if (list[i] === list[i - 1]) {
      for (let j = i + 1; j < list.length; j++) {
        if (list[j] !== list[i - 1] && (i === list.length - 1 || list[j] !== list[i + 1])) {
